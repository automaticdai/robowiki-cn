...
| 页面 | 主要内容 |
|------|---------|
| [ROS 1](ros1.md) | 架构概述、核心概念、发行版、安装与局限性 |
| [工作空间与软件包](ros1-workspace.md) | catkin 工作空间、CMakeLists.txt 与 package.xml、rosdep、Overlay |
| [通信机制](ros1-communication.md) | 话题、服务、动作、参数服务器、自定义消息、Nodelet |
| [节点编程](ros1-node-programming.md) | roscpp 与 rospy 节点模板、回调调度、多线程与生命周期 |
| [坐标变换、启动与数据录制](ros1-tf-launch-bag.md) | TF2、roslaunch XML、rosbag 录制与回放 |
| [URDF 机器人建模](ros1-urdf.md) | URDF 语法、xacro 宏、与 RViz/Gazebo/MoveIt 的衔接 |
| [Navigation Stack](ros1-navigation.md) | move_base、代价地图、全局与局部规划器、AMCL 定位 |
| [命令与排查速查](ros1-full-reference.md) | 命令速查表、核心软件包生态、常见问题排查、发行版时间线 |
| [ROS 2](ros2.md) | 下一代 ROS 的架构与迁移路径 |
