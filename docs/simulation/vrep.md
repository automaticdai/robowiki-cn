...
CoppeliaSim 章节按「功能概览 → 外部接口 → 机器人工作流 → 脚本与训练 → 性能 → 速查」的顺序组织：

| 页面 | 主要内容 |
|------|---------|
| [CoppeliaSim](vrep.md) | 核心特性、脚本语言支持、场景编辑器、内置模型、安装与版本 |
| [远程 API 与 ROS 2 集成](vrep-remote-api-ros2.md) | ZeroMQ Remote API、步进同步、ROS 2 插件、协同仿真架构 |
| [机器人工作流](vrep-robotics-workflows.md) | 移动机器人导航、机械臂抓取、多机器人、视觉传感器、CAD 导入 |
| [Lua 脚本与强化学习](vrep-scripting-rl.md) | 子脚本编写、回调时序、Gymnasium 环境封装与训练 |
| [性能优化与调试](vrep-performance-debugging.md) | 步长调优、物理引擎对比、碰撞检测优化、无头模式 |
| [API 与资源速查](vrep-full-reference.md) | API 体系、插件生态、模型库、仿真器对比、学习路径 |
